...
client.on("messageCreate", (message) => {
  if (message.author.bot) {
    return;
  } else if (message.content == "cabra") {
    message.reply("femboy");
  } else if (message.content == "gloglo") {
    const attachment = new AttachmentBuilder()
      .setFile("./assets/gloglo.gif")
      .setName("gloglo.gif");
...
client.on("interactionCreate", (interaction) => {
  ...
  if (interaction.commandName === "mas-gay") {
    const persona1 = interaction.options.get("primera-persona").value;
    const persona2 = interaction.options.get("segunda-persona").value;

    const masGay = buscarMasGay(persona1, persona2);
    ...
    fs.writeFile("./assets/jsons/penesChupados.json", content, (err) => {
      if (err) {
        console.error(err);
      } else {
